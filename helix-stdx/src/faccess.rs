...
pub fn chown(p: &Path, uid: Option<u32>, gid: Option<u32>) -> io::Result<()> {
    let uid = uid.map(|n|  rustix::fs::Uid::from_raw(n));
    let gid = gid.map(|n| rustix::fs::Gid::from_raw(n));
    rustix::fs::chown(p, uid, gid)?;
    Ok(())
}
...
pub fn copy_xattr(src: &Path, dst: &Path) -> io::Result<()> {
    ...
    let size = match rustix::fs::listxattr::<&std::path::Path, &mut [u8; 0]>(src, &mut [])? {
    ...
    for key in key_list[..size].split_inclusive(|&b| b == 0) {
        ...
        let attr_len = rustix::fs::getxattr::<&std::path::Path, &CStr, &mut [u8; 0]>(src, key, &mut [])?;
